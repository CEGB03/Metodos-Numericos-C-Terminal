...
#include "interpolacion_spline.h"
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#define FILAS 20
...
void buildMatrixSplineLineal(double m[FILAS][2], double a[FILAS][FILAS], double b[FILAS], int filas) {
	...
	for (int k = 0; k < n; k++) {
		// Ecuación del punto inicial del intervalo
		a[2*k][2*k] = m[k][0];
		a[2*k][2*k + 1] = 1;
		b[2*k] = m[k][1];

		// Ecuación del punto final del intervalo
		a[2*k + 1][2*k] = m[k+1][0];
		a[2*k + 1][2*k + 1] = 1;
		b[2*k + 1] = m[k+1][1];
	}
	...
	if (x >= m[0][0] && x <= m[fila - 1][0]) {
		for (int i = 0; i < fila - 1; i++) {
			if (x >= m[i][0] && x < m[i+1][0]) {
				resultado = z[2*i]*x + z[2*i+1];
				intervalo = i;
				break;
			}
		}
		printf("\nEl valor interpolado se encuentra en el intervalo %d", intervalo);
...
void buildMatrixSplineCuadratico(double m[FILAS][2], double a[FILAS][FILAS], double b[FILAS], int filas) {
	...
	for (int k = 0; k < n; k++) {
		// Ecuación del punto inicial del intervalo
		a[2*k][3*k] = pow(m[k][0], 2);
		a[2*k][3*k + 1] = m[k][0];
		a[2*k][3*k + 2] = 1;
		b[2*k] = m[k][1];

		// Ecuación del punto final del intervalo
		a[2*k + 1][3*k] = pow(m[k+1][0], 2);
		a[2*k + 1][3*k + 1] = m[k+1][0];
		a[2*k + 1][3*k + 2] = 1;
		b[2*k + 1] = m[k+1][1];
	}

	// Condiciones de continuidad de las primeras derivadas
	for (int k = 1; k < n; k++) {
		a[2*n + k - 1][3*(k-1)] = 2 * m[k][0];
		a[2*n + k - 1][3*(k-1) + 1] = 1;
		a[2*n + k - 1][3*k] = -2 * m[k][0];
		a[2*n + k - 1][3*k + 1] = -1;
		b[2*n + k - 1] = 0;
	}

	// Derivada segunda en los extremos igual a 0 (condiciones de frontera naturales)
	a[3*n - 2][0] = 2;
	a[3*n - 1][3*(n-1)] = 2;
	b[3*n - 2] = 0;
	b[3*n - 1] = 0;
...
void metodoSplineCuadratica(double m[FILAS][2], double z[FILAS], int fila) {
	...
	if (x >= m[0][0] && x <= m[fila - 1][0]) {
		for (int i = 0; i < fila - 1; i++) {
			if (x >= m[i][0] && x < m[i+1][0]) {
				resultado = z[3*i]*pow(x,2) + z[3*i+1]*x + z[3*i+2];
				intervalo = i;
				break;
			}
		}
...
void metodoSplineCubico (){
    ...
    triangulacionInterpolacionSpline(a, b, x, 4*(filas-1));
...
void triangulacionInterpolacionSpline(double a[FILAS][COLUMNAS], double b[FILAS], double x[FILAS], int filas){
	for (int i = 0 ; i < (filas - 1) ; i++){
		pivotInterpolacionSpline(a, b, filas , i);
	...
	double norma = determinanteInterpolacionSpline(a,b,x,filas);
	if(norma == 0.0){
		printf("\n\nmatriz singular");
	}else{
		retrostutitucionInterpolacionSpline(a, b, x, filas);
	}
}
